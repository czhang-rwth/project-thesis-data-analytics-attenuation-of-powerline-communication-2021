...
plt.title('Regression der Dämpfungen, 10m')
...
for tempstring in os.listdir(file10m):
    ...
    slope10, intercept10, r10, p10, std_err10 = stats.linregress(x, y)
    y_regress = [slope10*a + intercept10 for a in x]
    plt.plot(x, y_regress, color = temp_color[temp_index], linewidth = 0.075)
    plt.plot(x, y, color = temp_color[temp_index], linewidth = 0.075, alpha = 0.1)
    temp_index += 1
...
plt.savefig('E:\\Projektarbeit\\Data\\Kabelmessung_20211201\\Regression von Dämpfung 10m.png', dpi = 2000)
########################## 10m ###########################################################
...
plt.figure(figsize=(10,6))
plt.title('Regression der Dämpfungen, 15m')
plt.xlabel('Frequenz [MHz]', font1)
plt.ylabel('Dämpfung [dB]', font1)
plt.xticks(fontsize = 12, fontfamily = 'Arial')
plt.yticks(fontsize = 12, fontfamily = 'Arial')
...
for tempstring in os.listdir(file15m):
    ...
    slope15, intercept15, r15, p15, std_err15 = stats.linregress(x, y)
    y_regress = [slope15*a + intercept15 for a in x]
    plt.plot(x, y_regress, color = temp_color[temp_index], linewidth = 0.075)
    plt.plot(x, y, color = temp_color[temp_index], linewidth = 0.075, alpha = 0.1)
    temp_index += 1
...
custom_lines = [Line2D([0], [0], color='blue', lw=2),
                ...
                Line2D([0], [0], color='maroon', lw=2)]
plt.legend(custom_lines, ['T < 30 °C', '30 °C <= T < 40 °C', '40 °C <= T < 50 °C', '50 °C <= T < 60 °C', 'T >= 60 °C'], prop = font1, loc = 'upper right')
plt.savefig('E:\\Projektarbeit\\Data\\Kabelmessung_20211201\\Regression von Dämpfung 15m.png', dpi = 2000)
########################## 15m ###########################################################
...
for tempstring in os.listdir(file30m):
    ...
    slope30, intercept30, r30, p30, std_err30 = stats.linregress(x, y)
    y_regress = [slope30*a + intercept30 for a in x]
    plt.plot(x, y_regress, color = temp_color[temp_index], linewidth = 0.075)
    plt.plot(x, y, color = temp_color[temp_index], linewidth = 0.075, alpha = 0.1)
    temp_index += 1
...
custom_lines = [Line2D([0], [0], color='blue', lw=2),
                ...
                Line2D([0], [0], color='maroon', lw=2)]
plt.legend(custom_lines, ['T < 30 °C', '30 °C <= T < 40 °C', '40 °C <= T < 50 °C', '50 °C <= T < 60 °C', 'T >= 60 °C'], prop = font1, loc = 'upper right')
plt.savefig('E:\\Projektarbeit\\Data\\Kabelmessung_20211201\\Regression von Dämpfung 30m.png', dpi = 2000)
############################################################ 30m ################################################################