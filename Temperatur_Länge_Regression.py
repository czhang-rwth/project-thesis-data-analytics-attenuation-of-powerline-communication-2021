...
plt.title('Regression von Dämpfung(30m) - Dämpfung(15m)')
...
for temp30_file in os.listdir(file30m):
    ...
    for temp15_file in os.listdir(file15m):
        ...
        if temp15 == temp30:
            ...
            slope, intercept, r, p, std_err = stats.linregress(freq30, dampf_achse)
            dampf_regress = [slope*a + intercept for a in freq15]
            plt.plot(freq15, dampf_achse, lw = 0.2, alpha = 0.1)
            plt.plot(freq15, dampf_regress, lw = 0.2)

plt.savefig('E:\\Projektarbeit\\Data\\Kabelmessung_20211201\\30m-15m 2D Regression.png', dpi = 1500)
################################################### 30m - 15m ##########################################################################
...
plt.style.use('seaborn')
plt.figure(figsize=(10,6))
plt.title('Regression von Dämpfung(30m) - Dämpfung(10m)')
plt.xlabel('Frequenz [MHz]', font1)
plt.ylabel('Dämpfung [dB]', font1)
plt.xticks(fontsize = 12, fontfamily = 'Arial')
plt.yticks(fontsize = 12, fontfamily = 'Arial')
...
for temp30_file in os.listdir(file30m):
    ...
    for temp10_file in os.listdir(file10m):
        ...
        if temp10 == temp30:
            ...
            slope, intercept, r, p, std_err = stats.linregress(freq30, dampf_achse)
            dampf_regress = [slope*a + intercept for a in freq30]
            plt.plot(freq10, dampf_achse, lw=0.2, alpha=0.1)
            plt.plot(freq10, dampf_regress, lw = 0.2)

plt.savefig('E:\\Projektarbeit\\Data\\Kabelmessung_20211201\\30m-10m 2D regression.png', dpi = 1500)
################################################### 30m - 10m ##########################################################################
...
plt.style.use('seaborn')
plt.figure(figsize=(10,6))
plt.title('Regression von Dämpfung(15m) - Dämpfung(10m)')
plt.xlabel('Frequenz [MHz]', font1)
plt.ylabel('Dämpfung [dB]', font1)
plt.xticks(fontsize = 12, fontfamily = 'Arial')
plt.yticks(fontsize = 12, fontfamily = 'Arial')
...
for temp15_file in os.listdir(file15m):
    ...
    for temp10_file in os.listdir(file10m):
        ...
        if temp10 == temp15:
            ...
            slope, intercept, r, p, std_err = stats.linregress(freq15, dampf_achse)
            dampf_regress = [slope*a + intercept for a in freq15]
            plt.plot(freq10, dampf_achse, lw=0.2, alpha=0.1)
            plt.plot(freq10, dampf_regress, lw=0.2)

plt.savefig('E:\\Projektarbeit\\Data\\Kabelmessung_20211201\\15m-10m 2D Regression.png', dpi = 1500)
################################################### 15m - 10m ##########################################################################